...
- `CachedContract(addressOrName, contractInterface, signerOrProvider, cacheByBlockTag?)`: Creates a new `CachedContract` instance with address `addressOrName`, interface `contractInterface` and signer or provider `signerOrProvider`, similarly to `ethers.Contract`. If `signerOrProvider` is a provider, it instead uses as provider a `ProviderCache` proxy to the original provider with the specified `cacheByBlockTag` parameter; if it's a signer, it just uses the provided signer and works in the same way as an `ethers.Contract` (no caching).
- `from(contract, cacheByBlockTag?)`: Creates a new `CachedContract` instance from `contract`, an `ethers.Contract` instance by collecting its fields and calling the constructor. A wrapper to `new CachedContract(contract.address, contract.interface, contract.signer || contract.provider, cacheByBlockTag?)`.
